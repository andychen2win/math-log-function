...
import Head from 'next/head'; // Next.js 特有的 Head 组件
...
const callGemini = async (prompt, systemInstruction = "") => {
    try {
        // ✅ 修改：不再请求 googleapis，而是请求自己的后端 /api/gemini
...
const MathText = ({ children }) => {
    ...
    useEffect(() => {
        // Next.js 中，我们可以直接在 Head 中引入，但为了兼容原有逻辑，这里保持动态加载
            ...
            if (index % 2 === 0) return <span key={index}>{part}</span>;
                ...
                if (katexLoaded && window.katex) {
                    try {
                        ...
                        return <span key={index} dangerouslySetInnerHTML={{ __html: html }} className="mx-1 font-serif inline-block" />;
                    } catch (e) {
...
export default function Home() {
    ...
    return (
        <div className="flex flex-col h-screen bg-slate-50 font-sans text-slate-800 overflow-hidden">
            <Head>
                <title>高中数学：对数函数</title>
                <meta name="viewport" content="width=device-width, initial-scale=1" />
                {/* 在这里引入 Tailwind CSS，如果你没有设置 postcss */}
